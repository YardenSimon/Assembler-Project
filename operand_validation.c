...
int validate_operand(const char *op, OpCode opcode, int is_source) {
    ...
    if (method == -1) {
        add_error(ERROR_INVALID_OPERAND, current_filename, current_line_number,
                  "Invalid addressing method for operand: %s", op);
        return 0;
    }
    ...
    if (is_source) {
        if (!(opcode_addressing[opcode].source_allowed & (1 << (method - 1)))) {
            add_error(ERROR_INVALID_OPERAND, current_filename, current_line_number,
                      "Invalid source addressing method for opcode: %s", OPCODE_NAMES[opcode]);
            return 0;
        }
    } else {
        if (!(opcode_addressing[opcode].dest_allowed & (1 << (method - 1)))) {
            add_error(ERROR_INVALID_OPERAND, current_filename, current_line_number,
                      "Invalid destination addressing method for opcode: %s", OPCODE_NAMES[opcode]);
            return 0;
        }
    }
    return 1;
